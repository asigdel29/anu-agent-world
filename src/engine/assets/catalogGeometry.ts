 ...
 */
export function mergeGeometries(parts: readonly BufferGeometry[]): BufferGeometry {
...
type GeometryNode = Object3D & { readonly geometry: BufferGeometry };
...
export function collectCatalogGeometry(scene: Object3D, names: Iterable<string>): CatalogGeometry {
  ...
  for (const name of new Set(names)) {
    ...
    root.traverse((node) => {
      if (hasGeometry(node)) parts.push(bakeTransform(node, root));
    });
    if (parts.length === 0) continue;

    out.set(name, parts.length === 1 ? parts[0]! : mergeGeometries(parts));
  }
